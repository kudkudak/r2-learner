...
def make_datasets():
    return [make_moons(n_samples=500, noise=0.3, random_state=0),
            make_circles(n_samples=500, noise=0.2, factor=0.75, random_state=1),
            # make_classification(n_samples=500, n_classes=3, n_clusters_per_class=1, n_informative=2)]
...
#             "ELM(10,tribas)", "ELM(hardlim)", "ELM(20,rbf(0.1))"]
    names = ["ELM(10,tanh)", "ELM(10,lin)", "ELM(10,rect)",
             "ELM(10,softpl)", "ELM(hardlim)", "ELM(10,leakyrelu)"]
    ...
    srhl_rect = MLPRandomLayer(n_hidden=nh, activation_func=rectifier)

    linear = (lambda x: x)
    srhl_lin = MLPRandomLayer(n_hidden=nh, activation_func=linear)
    ...
    srhl_softplus = MLPRandomLayer(n_hidden=nh, activation_func=softplus)

    srhl_hardlim = MLPRandomLayer(n_hidden=nh, activation_func='hardlim')

    # use gaussian RBF
    leakyrelu = (lambda x: np.maximum(x, 0.1 * x))
    srhl_leakyrelu = MLPRandomLayer(n_hidden=nh, activation_func=leakyrelu)
    #srhl_rbf = RBFRandomLayer(n_hidden=nh*2, rbf_width=0.1, random_state=0)

    log_reg = LogisticRegression()

    classifiers = [GenELMClassifier(hidden_layer=srhl_tanh),
                   GenELMClassifier(hidden_layer=srhl_lin),
                   GenELMClassifier(hidden_layer=srhl_rect),
                   GenELMClassifier(hidden_layer=srhl_softplus),
                   GenELMClassifier(hidden_layer=srhl_hardlim),
                   GenELMClassifier(hidden_layer=srhl_leakyrelu)]
