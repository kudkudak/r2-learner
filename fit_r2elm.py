...
def fit_r2elm_grid(data, config, logger=None):
    ...
    cv_grid = GridSearchCV(model, param_grid=config['param_grid'], scoring=config['scoring'], n_jobs=-1, \
                           fit_params={'fit_h': config['fit_h']}, cv=config['cv'])
...
def fit_r2elm(data, config, logger=None) :
    ...
    for train_index, test_index in folds :
        ...
        model.fit(X_train, Y_train, fit_h=config['fit_h'])
...
def fit_r2elm_on_dataset(data, param_grid_in=None, grid_config_in=None, fold_config_in=None, to_file=False):
    ...
    grid_config = {'experiment_name': 'R2ELM grid_search_on_' + data.name,
    ...
    fold_config = {'experiment_name': 'R2ELM k-fold_testing_on_' + data.name,
