...
def fit_r2svm_grid(data, config, logger=None):
    """
...
def fit_grid_default_r2svm(data):
    ...
    config = {'experiment_type': 'grid',
                  'param_grid': default_r2svm_grid_parameters(),
                  'n_fold': 3,
                  'scoring': 'accuracy',
                  'store_clf': False,
                  'seed': None,
                  'experiment_name': 'default R2SVM grid experiment on ' + data.name}

    assert hasattr(data, 'name')
    assert hasattr(data, 'data')
    assert hasattr(data, 'target')

    return  fit_r2svm_grid(config, data.data, data.target)

def fit_r2svm_on_dataset(data, param_grid_in=None, grid_config_in=None, fold_config_in=None, to_file=False):
    ...
    grid_config = {'experiment_name': 'R2SVM grid_search_on_' + data.name,
...
def default_r2svm_grid_parameters() :
