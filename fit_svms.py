...
def fit_svc_grid(data, config, logger=None):
    """
    ...
    if config['refit'] :
        results['best_cls'] = SVC(**cv_grid.best_params_).fit(X,Y)
    else :
...
def fit_svc(data, config, logger=None) :
    """
...
def fit_svc_on_dataset(data, param_grid_in=None, grid_config_in=None, fold_config_in=None, to_file=False):

    scaler = MinMaxScaler()
    data.data = scaler.fit_transform(data.data)

    param_grid = default_svc_grid_parameters()
...
def default_svc_grid_parameters() :
